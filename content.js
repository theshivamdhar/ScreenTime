(function () {
  ...
  function handleInvalidation() {
    cleanup();
    console.log("ScreenTime: Extension context invalidated. Cleaning up...");
  }

  if (chrome.runtime && chrome.runtime.id) {
    try {
      chrome.runtime.onSuspend.addListener(handleInvalidation);
    } catch (e) {
      console.error("Error adding onSuspend listener:", e);
    }
  } else {
    console.warn(
      "ScreenTime: Chrome runtime not available. Extension may be in an invalid state."
    );
    handleInvalidation();
  }
})();
